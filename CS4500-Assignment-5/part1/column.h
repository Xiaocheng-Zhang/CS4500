...
public:
  ...
  void push_back(int i) { val_->append(i); }
};
...
public:
  ...
  ~BoolColumn() { delete val_; }
  bool get(size_t idx) {
    if (val_) {
      return val_->get_bool(idx);
    }
    return 0;
  }
...
public:
  ...
  ~FloatColumn() { delete val_; }

  float get(size_t idx) {
    if (val_) {
      return val_->get_float(idx);
    }
    return 0;
  }
...
public:
  ...
  Row *track_row(Schema *schema, size_t idx) {
    Row *temp = new Row(*schema);
    // std::cout<<idx<<"\n";
    for (size_t col = 0; col < schema->type_vec->size(); col++) {
      char type = schema->col_type(col);
      Column *c = get_Column(col);
      if (!c) {
        continue;
      }
      if (type == 'I') {
